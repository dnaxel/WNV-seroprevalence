...
birds.by.town <- read.xlsx("corvids_towns.xlsx")
emilia.borders <- merge(birds.by.town[,c("id", "TOTAL", "INCIDENCE", "SUBREGION")], cbind(n = 1:nrow(emilia.data), emilia.data), all=T)
...
p1A <- ggmap(map.emilia) +
  ...
  geom_point(data = birds.by.town, aes(x = long, y = lat, size = TOTAL, color = INCIDENCE)) +
...
generate.plots <- function(res, burn.in = 0, n.samples = 200, q  = 0.418, to.plot = T){
  N <- nrow(res)
  ...
  for(s in subregions){
    ...
    if(to.plot){
      ...
      p.sero <- ggplot(data = z.plot) + 
        ...
        labs(title="", caption="") + scale_x_continuous(breaks = years+0.5, limits = c(2013, 2023), labels = paste("1st", "July", years, sep=" ")) +
...
outputs <- generate.plots(as.data.frame(mcmc.sample), burn.in = 50000, n.samples = 200)
...
fig4 <- plot_grid(outputs$seroprevalence.plots[[1]], 
                ...
                nrow = 3)
ggsave("Fig4.jpeg", fig4, height = 10, width = 10)
...
fig_names <- c("FigC.jpeg", "Fig5.jpeg", "FigD.jpeg")
for(i in subregions){
